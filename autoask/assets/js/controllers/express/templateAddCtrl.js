app.controller("expressTemplateAddCtrl", ["$scope", "autoaskService", "ngTableParams", "$location", function ($scope, autoaskService, ngTableParams, $location) {
    $scope.init = function () {
        $scope.express = {};
        //表单数据
    };
    ...
    var submit = function () {
        $scope.loadingPromise = autoaskService.insertExpressTemplate($scope.express, function () {
            swal('创建快递模板成功', '', 'success');
    ...
    $scope.checkedName = [];
    //地区(省)checkBox封装
    autoaskService.getTemplateProvinceList(null, function (resp) {
