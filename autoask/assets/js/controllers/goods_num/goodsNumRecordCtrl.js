app.controller("goodsNumRecordListCtrl", ["$scope", "ngTableParams", "autoaskService", function ($scope, ngTableParams, autoaskService) {
    ...
    $scope.init = function () {
        ...
        // 加载合作商列表
        $scope.loadingPromise = autoaskService.getPartnerList({page: 1, count: 10000}, function (resp) {
            $scope.partners = resp.data.result;
        });
        // 加载工厂列表
...
app.controller("goodsNumRecordChangeCtrl", ["$scope", "autoaskService", '$location', function ($scope, autoaskService, $location) {
    $scope.init = function () {
        ...
        $scope.loadingPromise = autoaskService.getFactoryList({page: 1, count: 100000}, function (resp) {
            $scope.factoryList = resp.data.result;
        });

        $scope.loadingPromise = autoaskService.getPartnerList({page: 1, count: 100000}, function (resp) {
            $scope.partnerList = resp.data.result;
        });

    };

    $scope.merchantTypeChanged = function () {
        if ($scope.data.merchantType === 'partner') {
            $scope.merchantList = $scope.partnerList;
        } else if ($scope.data.merchantType === 'factory') {
            $scope.merchantList = $scope.factoryList;
        }
    };
