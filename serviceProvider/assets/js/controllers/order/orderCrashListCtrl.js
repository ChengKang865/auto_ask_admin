 ...
 * 线下预约订单列表页面控制器
 */
app.controller('orderCrashListCtrl', function ($scope, $rootScope, autoaskService, ngTableParams) {
    // 初始化方法
    $scope.init = function () {
        $scope.merchants = [];
        $scope.criteria = {
    ...
    $scope.refuseCash = function (orderId) {
        $scope.loadingPromise = autoaskService.refuseCash({
            orderId: orderId
        }, function () {
            swal("拒绝成功", "", "success");
            $scope.tableParams.reload();
    ...
    $scope.confirmCash = function (orderId) {
        $scope.loadingPromise = autoaskService.confirmCash({
            orderId: orderId
        }, function () {
            swal("确认收到现金成功", "", "success");
            $scope.tableParams.reload();
        });
    }
